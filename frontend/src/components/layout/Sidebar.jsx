...
function NavItem({ to, icon, label, end, collapsed, unreadCount }) {
  ...
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        `flex items-center justify-between px-3.5 py-3 rounded-xl text-sm font-semibold transition-all group relative ${
          isActive
    ...
    >
      ...
      {isAnnouncement && unreadCount > 0 && (
        <span className={`${collapsed ? 'absolute top-2 right-2' : ''} px-2 py-0.5 rounded-full text-[10px] font-black bg-rose-600 text-white shrink-0`}>
      ...
      {/* Collapsed Tooltip */}
      {collapsed && (
        <div className="absolute left-16 top-1/2 -translate-y-1/2 ml-2 px-3 py-1.5 bg-slate-950 text-white text-[11px] font-bold rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 shadow-lg border border-slate-800 z-50 whitespace-nowrap flex items-center gap-2">
          <span>{label}</span>
          {isAnnouncement && unreadCount > 0 && (
