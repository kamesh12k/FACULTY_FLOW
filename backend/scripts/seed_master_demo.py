...
def seed_master():
    db = SessionLocal()
    try:
        print("Starting FAFLOW Complete Production-Grade College Demo Seeding...")
        ...
        # Day Orders 1 to 6 sequence: 2026-09-10 (Thursday) -> DO 2
        # Sep 1: DO 1, Sep 2: DO 2, Sep 3: DO 3, Sep 4: DO 4, Sep 7: DO 5, Sep 8: DO 6, Sep 9: DO 1, Sep 10: DO 2
        ...
        while cur_date <= date(2026, 12, 31):
            ...
            elif cur_date == date(2026, 10, 2): # Gandhi Jayanti
                if not cal_day:
                    db.add(CalendarDay(date=cur_date, day_type=DayType.government_holiday, day_order=None, label="Gandhi Jayanti", academic_year_id=ay.id, semester_id=sem.id))
            else:
        ...
        for emp_id, name, email, d_code, base_credit in teacher_roster:
            ...
            # Initialize teacher preference if not exists
            pref = db.query(SubstitutionPreference).filter(SubstitutionPreference.teacher_id == t_user.id).first()
            if not pref:
                pref = SubstitutionPreference(
                    teacher_id=t_user.id,
                    accept_auto_assignments=True,
                    allow_emergency_assignments=True,
                    only_my_classes=False,
                    max_weekly_substitutions=5
                )
                db.add(pref)
                db.commit()
        ...
        # Substitute Teacher Availability on Day Order 2:
        # Dr. Priya Sharma (FAC002) is free at P1 (teaches CS201 at P2, P4)
        for do, p, sid, cid, rid in [(2, 2, subjects["CS201"].id, classes["I CSE-A"].id, rooms[3].id), (2, 4, subjects["CS201"].id, classes["I CSE-B"].id, rooms[4].id)]:
        ...
        # Mr. Karthik Raj (FAC003) is free at P3 (teaches CS204 at P1, P5)
        for do, p, sid, cid, rid in [(2, 1, subjects["CS204"].id, classes["IV CSE-A"].id, rooms[5].id), (2, 5, subjects["CS204"].id, classes["IV CSE-B"].id, rooms[6].id)]:
        ...
        # Ms. Divya Raman (FAC004) is free at P5 (teaches CS302 at P2, P3)
        for do, p, sid, cid, rid in [(2, 2, subjects["CS302"].id, classes["III CSE-A"].id, rooms[7].id), (2, 3, subjects["CS302"].id, classes["I CSE-A"].id, rooms[8].id)]:
        ...
        # Populate remaining slots across all teachers & classes
        dept_subjects = {c: [s for s in subjects.values() if s.department_id == departments[c].id] for c in departments}
        dept_classes = {c: [cl for cl in classes.values() if cl.department_id == departments[c].id] for c in departments}
        dept_teachers = {c: [t for t in teachers.values() if t.department_id == departments[c].id] for c in departments}
        ...
        for d_code, t_list in dept_teachers.items():
            ...
            for t in t_list:
                ...
                attempts = 0
                while current_slots < 14 and attempts < 150:
                    attempts += 1
                    do = random.randint(1, 6)
                    p = random.randint(1, 5)
                    if (t.id, do, p) in teacher_busy:
                        continue
                    avail_classes = [c for c in d_clss if (c.id, do, p) not in class_busy]
                    if not avail_classes:
                        continue
                    avail_rooms = [r for r in rooms if (r.id, do, p) not in room_busy]
                    if not avail_rooms:
                        continue
                    cls_choice = random.choice(avail_classes)
                    room_choice = random.choice(avail_rooms)
                    subj_choice = random.choice(d_subs)

                    slot = TimetableSlot(teacher_id=t.id, subject_id=subj_choice.id, class_id=cls_choice.id, room_id=room_choice.id, day_order=do, period_number=p)
                    db.add(slot)
        ...
        # -------------------------------------------------------------
        # 8. CREDIT TRANSACTIONS FOR DR. ARUN KUMAR & OTHERS
        # -------------------------------------------------------------
        db.query(CreditTransaction).delete()
        db.commit()

        # Meaningful credit transaction ledger for Dr. Arun Kumar
        db.add(CreditTransaction(teacher_id=arun.id, change=5, reason="Initial semester allocated casual leave quota", category="manual_adjustment", created_at=datetime(2026, 8, 1, 9, 0)))
        db.add(CreditTransaction(teacher_id=arun.id, change=1, reason="Substitution coverage: Period 2 for Dr. Priya Sharma (CS201)", category="substitute_class", created_at=datetime(2026, 8, 12, 10, 15)))
        db.add(CreditTransaction(teacher_id=arun.id, change=-1, reason="Approved leave deduction: 14 Aug 2026 (Period 4)", category="penalty", created_at=datetime(2026, 8, 14, 11, 30)))
        db.add(CreditTransaction(teacher_id=arun.id, change=1, reason="Substitution coverage: Period 3 for Mr. Karthik Raj (CS204)", category="substitute_class", created_at=datetime(2026, 8, 22, 11, 30)))
        db.add(CreditTransaction(teacher_id=arun.id, change=-1, reason="Approved leave deduction: 26 Aug 2026 (Period 1)", category="penalty", created_at=datetime(2026, 8, 26, 8, 30)))
        db.commit()

        # -------------------------------------------------------------
        # 9. LEAVE HISTORY FOR DR. ARUN KUMAR & FACULTY
        # -------------------------------------------------------------
        db.query(AlterAssignment).delete()
        db.query(LeaveRequest).delete()
        db.commit()

        # Scenario A: Approved Past Leave for Dr. Arun Kumar (14 Aug)
        l1 = LeaveRequest(teacher_id=arun.id, date=date(2026, 8, 14), day_order=6, period_number=4, reason="Medical consultation", status=LeaveStatus.approved)
        db.add(l1)
        db.commit()
        db.refresh(l1)
        db.add(AlterAssignment(leave_request_id=l1.id, substitute_teacher_id=priya.id, assignment_type=AssignmentType.faculty_recommended, compatibility_score=95.0))

        # Scenario B: Cancelled Past Leave for Dr. Arun Kumar (20 Aug)
        l2 = LeaveRequest(teacher_id=arun.id, date=date(2026, 8, 20), day_order=3, period_number=2, reason="Personal errand", status=LeaveStatus.cancelled)
        db.add(l2)

        # Scenario C: Rejected Past Leave for Dr. Arun Kumar (28 Aug)
        l3 = LeaveRequest(teacher_id=arun.id, date=date(2026, 8, 28), day_order=1, period_number=1, reason="Short notice workshop", status=LeaveStatus.rejected)
        db.add(l3)

        # Historical leaves for other teachers
        db.add(LeaveRequest(teacher_id=teachers["FAC005"].id, date=date(2026, 8, 18), day_order=2, period_number=2, reason="Conference presentation", status=LeaveStatus.approved))
        db.add(LeaveRequest(teacher_id=teachers["FAC009"].id, date=date(2026, 8, 22), day_order=4, period_number=3, reason="Family event", status=LeaveStatus.approved))
        db.add(LeaveRequest(teacher_id=teachers["FAC015"].id, date=date(2026, 8, 25), day_order=6, period_number=1, reason="Medical leave", status=LeaveStatus.approved))
        db.commit()

        # -------------------------------------------------------------
        # 10. REALISTIC NOTIFICATIONS FOR DR. ARUN KUMAR
        # -------------------------------------------------------------
        db.query(Notification).delete()
        db.commit()

        notifs = [
            (arun.id, "Leave Request Approved", "Your leave request for 14 Aug 2026 (Period 4) has been approved by administration.", "leave_approved", l1.id, False),
            (arun.id, "Substitute Assigned", "Dr. Priya Sharma has been confirmed as your substitute for 14 Aug (Period 4).", "substitute_assigned", l1.id, False),
            (arun.id, "Coverage Bonus Earned", "You earned +1 Credit for handling substitution coverage for CS204 on 22 Aug.", "credit_update", None, True),
            (arun.id, "Leave Request Cancelled", "Your leave request for 20 Aug 2026 has been successfully cancelled.", "leave_cancelled", l2.id, True),
            (arun.id, "Academic Notice", "Internal assessment timetables for Odd Semester 2026 are now live.", "system_announcement", None, True),
        ]
        for uid, title, body, etype, r_id, is_r in notifs:
            db.add(Notification(user_id=uid, title=title, body=body, event_type=etype, related_leave_id=r_id, is_read=is_r))
        db.commit()

        print("MASTER DEMO SEEDING COMPLETED SUCCESSFULLY!")
        print(f"Demo Account: Dr. Arun Kumar ({arun.email}) | Password: Password123")
        print(f"Current Credit Balance: +{arun.credit_balance.balance}")

    except Exception as e:
        db.rollback()
        print(f"Error in master seeding: {e}")
        raise e
    finally:
        db.close()
