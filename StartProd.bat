...
echo %BLUE%========================================================================%RESET%
echo %BG_BLUE%                      FAFLOW - PRODUCTION DEPLOYER                      %RESET%
echo %BLUE%========================================================================%RESET%
echo.
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "Start-Process powershell -ArgumentList '-NoProfile', '-ExecutionPolicy', 'Bypass', '-NoExit', '-Command', '$host.UI.RawUI.WindowTitle = ''FAFLOW_BACKEND_PROD''; cd ''!ROOT_DIR!\backend''; while ($true) { Write-Host ''=== Starting Production Backend (uvicorn) ==='' -ForegroundColor Green; & .\venv\Scripts\python.exe -m uvicorn app.main:app --host 0.0.0.0 --port 8000 2>&1 | Tee-Object -FilePath ..\logs\backend_prod.log; Write-Host ''=== Production Backend crashed. Restarting in 5 seconds... ==='' -ForegroundColor Red; Start-Sleep -s 5 }'"

echo   - Starting production frontend web server on http://localhost:5173...
powershell -NoProfile -ExecutionPolicy Bypass -Command "Start-Process powershell -ArgumentList '-NoExit', '-Command', '$host.UI.RawUI.WindowTitle = ''FAFLOW_FRONTEND_PROD''; cd ''!ROOT_DIR!''; while ($true) { Write-Host ''=== Starting Production Frontend (serve) ==='' -ForegroundColor Green; cmd /c ''npx -y serve -s frontend/dist -l 5173 2>&1'' | Tee-Object -FilePath logs\frontend_prod.log; Write-Host ''=== Production Frontend stopped. Restarting in 5 seconds... ==='' -ForegroundColor Red; Start-Sleep -s 5 }'"
